...
public class WardenHeadRenderer implements BlockEntityRenderer<EDSkullBlockEntity> {
    ...
    @Override
    public void render(EDSkullBlockEntity entity,
                       float partialTicks,
        ...
        Direction facing = state.getValue(EDWallScullBlock.FACING);
...
//            case NORTH -> 0.0F;
        ...
        poseStack.translate(0.5F - (float)facing.getStepX() * 0.1875F, -0.8125F, 0.5F - (float)facing.getStepZ() * 0.1875F);
        poseStack.scale(-1.0F, -1.0F, 1.0F);

        VertexConsumer vertex = buffer.getBuffer(RenderType.entityCutoutNoCull(TEXTURE));
        poseStack.mulPose(Axis.YP.rotationDegrees(yRot));
        head.render(poseStack, vertex, light, overlay);
